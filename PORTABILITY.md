...
| Intent execution (`/monorepo-harness-intent-execute`) | `core/skills/intent-workflow/SKILL.md` "Workflow — Execute" + `core/scripts/tracker-issue.sh` (issue creation, `gh`) | `.claude/commands/monorepo-harness-intent-execute.md` → `/monorepo-harness-intent-execute`; issues created by the `.claude/agents/tracker.md` subagent | `.opencode/commands/monorepo-harness-intent-execute.md` → `/monorepo-harness-intent-execute`; the shared skill reaches opencode by path from the command, so no new `instructions` entry is needed | `.agents/skills/monorepo-harness-intent-execute/SKILL.md` → `/monorepo-harness-intent-execute`; issues created inline in the main session (no subagent primitive) | run `tracker-issue.sh --create` directly and follow the skill's Execute section by hand — the consent questions, workspace/phase derivation and issue creation are script- and instruction-driven, agent-agnostic |
...
- **Intent execution plans and records work; it never implements it, on any agent.**
  `/monorepo-harness-intent-execute` verifies the approval, confirms the workspace scope, splits the
...
- **No tracker credential is ever installed, asked for, or stored, on any agent.** The harness uses
  ...
  ships neither.
- **Changeset bumps are user policy on every agent.** `/monorepo-harness-changeset` proposes bump
  levels from a heuristic, but the package list, each bump, and the summary are always confirmed by
  the user before `draft-changeset.sh --write` runs. There is no per-agent variance here: on every
  adapter the consent loop is identical, only the entry-point mechanism differs. The underlying file
  production (`draft-changeset.sh`) is script-driven and agent-neutral, so the agent-agnostic
  fallback (run the script by hand) produces byte-identical output.
- **Knowledge-base ingest is task-end + gate-driven on every agent.** `kb-ingest.sh` performs the
  deterministic parts (structure, catalog rows, log entry, ADR copies, page skeletons); the prose is
  authored by whichever agent ran `-build`. The `check-kb` gate is wired into
  `core/scripts/memory-gate.sh` — the same universal hard gate as memory/verify — so every adapter
  (or no adapter) enforces "write memory ⇒ the compiled KB is current too."
