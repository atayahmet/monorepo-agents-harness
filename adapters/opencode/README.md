...
--sync-only`, plus `install-adapter.sh opencode --refresh`), reconcile the root `AGENTS.md` behind
its own second approval, audit, and clean up. No file is ever copied by hand.

With no adapter installed, paste that same prompt from `core/prompts/harness-update.md` into opencode
