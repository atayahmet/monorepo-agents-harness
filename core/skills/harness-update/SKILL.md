---
name: harness-update
description: Check whether the installed agent harness is up to date and, with user consent, upgrade it. Use when the user invokes /tah:update (Claude Code) or /tah-update (opencode/Codex CLI), or asks to check/update the harness, when a task touches monorepo-agents-harness/** and a version mismatch matters, or before starting work after a known harness release.
---
...
The version engine is `.agents/monorepo-agents-harness/scripts/harness-update.sh` (git + coreutils only; exit 0 = current, 1 = update available, 2 = unknown/unreachable). The script only performs version checks; it does **not** execute upgrades.

The actual upgrade is performed by you, the active agent, by reading the changelog prompts in `monorepo-agents-harness/changelogs/version-X.Y.Z.md`.

> **Installed layout (0.3.0+).** The bundle (`monorepo-agents-harness/`) remains the source of
> truth for upgrades, but the runtime-facing shared files (skills, scripts, governance docs) live
> under `.agents/monorepo-agents-harness/` as symlinks into the bundle. The version file is at
...
1. **Check** (from the repo root):
   ```bash
   bash .agents/monorepo-agents-harness/scripts/harness-update.sh check --json
   ```
   Parse `status`:
...
3. **Select the changelog prompts** to apply.
    ...
    - Read `monorepo-agents-harness/core/VERSION` from the downloaded bundle to determine the target version (`latest`).
...
7. **On consent, execute the plan**:
    - **Copy**: For each listed path, copy from `.harness-update-v<latest>/...` to `monorepo-agents-harness/...`. Directories ending in `/` are copied recursively and replace the target directory entirely.
    - **Delete**: Remove each listed path from `monorepo-agents-harness/...`. If the prompt says "(only if they exist)", skip silently when missing; otherwise treat a missing target as an error.
...
- **Pre-versioning install** (no `.agents/monorepo-agents-harness/VERSION` and no legacy `monorepo-agents-harness/core/VERSION`, exit status outdated with empty installed): treat as "very old install" — recommend following INSTALL.md §4 afresh instead of upgrading.
